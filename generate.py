...
def generate_image(arguments: [str]) -> None:
    # global torch
    ...
    vq_parser.add_argument("-o", "--output", type=str, help="Output image filename", default="output.png",
                           dest='output')
    ...
    vq_parser.add_argument("-cd", "--cuda_device", type=str, help="Cuda device to use", default="cuda:0",
                           dest='cuda_device')
    # Timeline
    vq_parser.add_argument("-tf", "--timeline_file", type=str, help="File with timeline data", default=None,
                           dest='timeline_file')
    # Execute the parse_args() method
    args = vq_parser.parse_args(arguments)
        ...
        def __init__(self, cut_size, cutn, cut_pow=1.):
            ...
            for item in args.augments[0]:
                if item == 'Ji':
                    ...
                    augment_list.append(K.RandomAffine(degrees=30, translate=0.1, shear=5, p=0.7, padding_mode='zeros',
                                                       keepdim=True))  # border, reflection, zeros
                elif item == 'Et':
                    augment_list.append(K.RandomElasticTransform(p=0.7))
                elif item == 'Ts':
            ...
            while True:
                ...
                if args.timeline_file:
                    ...
                    if timeline_frame_i >= timeline.iterations[timeline_point]:
                        ...
                        if timeline.zoom[timeline_point] != 1 or timeline.zoom_shift_x[timeline_point] != 0 or \
                                timeline.zoom_shift_y[timeline_point] != 0:
                            ...
                            if timeline.zoom_shift_x[timeline_point] != 0 or timeline.zoom_shift_y[timeline_point] != 0:
                                ...
                                pil_image_zoom = ImageChops.offset(pil_image_zoom,
                                                                   timeline.zoom_shift_x[timeline_point],
                                                                   timeline.zoom_shift_y[timeline_point])
